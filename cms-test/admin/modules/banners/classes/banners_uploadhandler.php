...
//
class banners_uploadhandler extends uploadhandler {
	protected $folder_name = 'banners'; //used throughout
	...
	{  
		...
		if($this->is_image($imgname))
		{
			...
			logged_query("INSERT INTO `banners` (`name`, `alt`, `link`, `desc`, `html_desc`,`posn`,`date`) VALUES (:imgname,:imgname,'','','',0, NOW())",0,array(
				":imgname" => $imgname
			));
	...
	{
		logged_query("DELETE FROM `banners` WHERE `name` =:imgname",0,array(":imgname" => $imgname) );
	}
