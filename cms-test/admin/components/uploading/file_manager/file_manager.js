...
var args = top.tinymce.activeEditor.windowManager.getParams(),
	...
	target_id_field = args.target_id_field,
...
resultLI +=	"</div>";
		...
		start: function (e){
			$('#total_progress').show();
			if (typeof XDomainRequest !== "undefined") writeMessage("Uploading"); // ie 8& 9 use this
		},
		stop: function (e) {
			$('#total_progress').css('width',0).hide();
			if (typeof XDomainRequest !== "undefined") writeMessage("Upload Complete: Re-open Popup to use it");
		...
		add: function (e, data) {
			...
			if(data.files[0].size < max_file_size*1024*1024)
			{	
			...
			else
			{
				...
				//set error message
				writeMessage(data.files[0].name + " exceeds "+ max_file_size + "MB Size Limit", 'error');
			}
		},
...
function doTipTips(){
	...
	$('.name').tipTip({
		defaultPosition:'top',
		content:"Click to Change Alt Tags"
	});
}	
...
{
	var file = $(that).closest('.file_wrap').attr('data-name');
...
function writeMessage(message,type)
{
	var error_class = type == 'error' ? 'error' : '',
		$messages = $('#messages');
	$('#messages li').removeClass('curMsg');
	if($('#messages li').length > 5) $('#messages li:first-child').next().remove();
	$messages.append('<li class="curMsg '+error_class+'">'+message+'</li>');
}
